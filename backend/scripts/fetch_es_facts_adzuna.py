"""Fetch ES salary facts from live job ads via the Adzuna API (Spain).

Per-profession upgrade over the INE group facts (fetch_es_facts): where enough
salaried ads exist, the profession gets its own p25-p75 from the open hiring
market; thin professions keep the INE group figure (seed order: INE first,
this file after).

Method:
- per slug, 1-2 title_only queries (seed/adzuna_queries_es.json; Spanish title
...
- garbage guards: ads whose title names another country are dropped (boards
  carry 'Enfermero/a en Suiza' style foreign offers), plausibility window
  12k-150k EUR/yr (mis-parsed monthly rates show up as 300k+);
- aggregate p25/median/p75; slugs with n < --min-n are SKIPPED (keep INE).

Auth: ADZUNA_APP_ID / ADZUNA_APP_KEY from env (/etc/astrolab/env on prod).
Trial tier limits ~25 req/min, ~250/day - the script paces itself and one full
run fits the daily budget. Run manually:

    python -m scripts.fetch_es_facts_adzuna --compare seed/facts_es.json \
        --out seed/facts_es_adzuna.json [--slugs a,b] [--pages 3] [--min-n 25]

Then: python -m scripts.seed_facts --file seed/facts_es_adzuna.json
"""
...
API = "https://api.adzuna.com/v1/api/jobs/es/search/{page}"
UA = "Astrolab/1.0 (diamexx@mail.ru)"
PAUSE = 2.6  # seconds between calls (trial: 25/min)

SAL_MIN, SAL_MAX = 12_000, 150_000  # plausibility window, EUR/year
# ads recruiting FOR another country, posted on Spanish boards
FOREIGN = re.compile(
    r"\b(suiza|alemania|francia|noruega|irlanda|belgica|bélgica|austria|"
    r"paises bajos|países bajos|holanda|luxemburgo|portugal|reino unido|"
    r"emiratos|qatar|arabia|extranjero)\b",
    re.IGNORECASE,
)
...
def ads_for(
    ...
    """{ad_id: salary_mid} for salaried, Spain-located, plausible ads."""
    out: dict[str, float] = {}
    for page in range(1, pages + 1):
        ...
        for r in results:
            ...
            if FOREIGN.search(title):
                continue
            # per-slug pollution guard (e.g. 'arquitecto' must not match
            # software architects, 'animador' not hotel entertainers)
...
def main() -> None:
    ...
    ap.add_argument("--queries", default=str(ROOT / "seed" / "adzuna_queries_es.json"))
    ap.add_argument("--compare", default=str(ROOT / "seed" / "facts_es.json"))
    ...
    rows, skipped = [], []
    print(f"{'slug':34} {'n':>5}  {'adzuna p25-p75 EUR/yr':>22}  {'INE low-high':>18}  ratio")
    ...
    out = {
        ...
        "source_name": "Adzuna live job-ad salary analysis (p25-p75, gross annual, Spain)",
        "source_url": "https://www.adzuna.es/",
        "as_of_date": today,
        "facts": rows,
    }
