"""Add French (locale='fr') i18n rows to the EXISTING question bank (FR-1).

    python -m scripts.seed_questions_fr [--file seed/questions_fr.json] [--version N]
...
DEFAULT_SEED = SEED_DIR / "questions_fr.json"
LOCALE = "fr"
...
async def run(seed_path: Path, version: int | None) -> None:
    ...
    async with SessionLocal() as s:
        ...
        if version is None:
            print("[seed_questions_fr] no question bank present — seed the RU bank first")
            return
        ...
        fr_by_qid = {r.question_id: r for r in fr_rows}
        ...
        for item in data["riasec"]:
            ...
            if dim != item["dimension"]:
                print(
                    f"[seed_questions_fr] WARN dimension mismatch for {item['ru']!r}: "
        ...
        await s.commit()
        print(
            f"[seed_questions_fr] version {version}: "
            f"+{inserted} inserted, ~{updated} updated, ={skipped} unchanged"
        )
        if missing:
            print(f"[seed_questions_fr] UNMATCHED ({len(missing)}): {missing[:6]}")
