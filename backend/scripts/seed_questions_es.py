"""Add Spanish (locale='es') i18n rows to the EXISTING question bank (ES-1).

    python -m scripts.seed_questions_es [--file seed/questions_es.json] [--version N]

Same contract as seed_questions_en (additive, idempotent, never wipes):
...
DEFAULT_SEED = SEED_DIR / "questions_es.json"
LOCALE = "es"
...
async def run(seed_path: Path, version: int | None) -> None:
    ...
    async with SessionLocal() as s:
        ...
        if version is None:
            print("[seed_questions_es] no question bank present — seed the RU bank first")
            return
        ...
        es_by_qid = {r.question_id: r for r in es_rows}
        ...
        for item in data["riasec"]:
            ...
            if dim != item["dimension"]:
                print(
                    f"[seed_questions_es] WARN dimension mismatch for {item['ru']!r}: "
        ...
        await s.commit()
        print(
            f"[seed_questions_es] version {version}: "
            f"+{inserted} inserted, ~{updated} updated, ={skipped} unchanged"
        )
        if missing:
            print(f"[seed_questions_es] UNMATCHED ({len(missing)}): {missing[:6]}")
