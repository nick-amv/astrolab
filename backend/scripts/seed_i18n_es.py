"""Add Spanish (locale='es') i18n rows to EXISTING catalog occupations (ES-1).

    python -m scripts.seed_i18n_es [--files seed/i18n_es_batchA.json,...]
...
DEFAULT_FILES = "seed/i18n_es_batchA.json,seed/i18n_es_batchB.json,seed/i18n_es_batchC.json"
...
async def run(paths: list[Path]) -> None:
    ...
    async with SessionLocal() as s:
        ...
        es_rows = (
            await s.execute(select(OccupationI18n).where(OccupationI18n.locale == "es"))
        ).scalars().all()
        es_by_occ = {r.occupation_id: r for r in es_rows}
        ...
        await s.commit()
        print(f"[seed_i18n_es] +{inserted} inserted, ~{updated} updated, ={skipped} unchanged")
        if missing:
            print(f"[seed_i18n_es] no occupation for: {', '.join(missing)}")
