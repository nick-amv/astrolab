"""Fetch DE salary facts from live job ads via the Adzuna API (Germany).

Per-profession upgrade over the Entgeltatlas group/median base (fetch_de_facts):
where enough salaried ads exist, the profession gets its own p25-p75 from the
open hiring market; thin professions keep the base figure (seed order: base
first, this file after).

Method:
- per slug, 1-2 title_only queries (seed/adzuna_queries_de.json; German title
...
- garbage guards: ads whose title names another country are dropped, plausibility
  window 15k-200k EUR/yr (mis-parsed monthly rates show up as 300k+);
- aggregate p25/median/p75; slugs with n < --min-n are SKIPPED (keep base).

Auth: ADZUNA_APP_ID / ADZUNA_APP_KEY from env (/etc/astrolab/env on prod).
Run manually (Germany is data-rich; a full run takes a while — background it):

    python -m scripts.fetch_de_facts_adzuna --compare seed/facts_de.json \
        --out seed/facts_de_adzuna.json [--slugs a,b] [--pages 3] [--min-n 18]

Then: python -m scripts.seed_facts --file seed/facts_de_adzuna.json
"""
...
SAL_MIN, SAL_MAX = 15_000, 200_000  # plausibility window, EUR/year
# ads recruiting FOR another country, posted on German boards
FOREIGN = re.compile(
    r"\b(schweiz|österreich|oesterreich|luxemburg|niederlande|belgien|"
    r"frankreich|italien|spanien|polen|tschechien|dänemark|norwegen|schweden|"
    r"vereinigtes königreich|großbritannien|england|usa|kanada|"
    r"vereinigte arabische emirate|katar|ausland|expat)\b",
    re.IGNORECASE,
)
...
def ads_for(
    ...
    """{ad_id: salary_mid} for salaried, Germany-located, plausible ads."""
    out: dict[str, float] = {}
    for page in range(1, pages + 1):
        ...
        for r in results:
            ...
            if FOREIGN.search(title):
                continue
            # per-slug pollution guard (e.g. 'Architekt' must not match
...
def main() -> None:
    ...
    ap.add_argument("--queries", default=str(ROOT / "seed" / "adzuna_queries_de.json"))
    ap.add_argument("--compare", default=str(ROOT / "seed" / "facts_de.json"))
    ap.add_argument("--out", default=str(ROOT / "seed" / "facts_de_adzuna.json"))
    ...
    rows, skipped = [], []
    print(f"{'slug':34} {'n':>5}  {'adzuna p25-p75 EUR/yr':>22}  {'base low-high':>18}  ratio")
