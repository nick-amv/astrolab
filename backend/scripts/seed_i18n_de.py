"""Add German (locale='de') i18n rows to EXISTING catalog occupations (DE-1).

    python -m scripts.seed_i18n_de [--files seed/i18n_de_batchA.json,...]
...
DEFAULT_FILES = "seed/i18n_de_batchA.json,seed/i18n_de_batchB.json,seed/i18n_de_batchC.json"
...
async def run(paths: list[Path]) -> None:
    ...
    async with SessionLocal() as s:
        ...
        de_rows = (
            await s.execute(select(OccupationI18n).where(OccupationI18n.locale == "de"))
        ).scalars().all()
        de_by_occ = {r.occupation_id: r for r in de_rows}
        ...
        await s.commit()
        print(f"[seed_i18n_de] +{inserted} inserted, ~{updated} updated, ={skipped} unchanged")
        if missing:
            print(f"[seed_i18n_de] no occupation for: {', '.join(missing)}")
